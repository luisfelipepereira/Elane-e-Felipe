const timerRoot = document.querySelector("#cronometro");
...
const letterTemplate = document.getElementById("letter-content-template");
...
const relationshipStartRaw = (timerRoot && timerRoot.dataset.startDate) || "2025-04-13T16:30:00";
...
function initRomanticLetterTypewriter() {
  ...
  const letterRawText = (letterTemplate && letterTemplate.textContent ? letterTemplate.textContent : "").replace(/\r/g, "").trim();

  if (!letterRawText) {
    return;
  }

  const formatLetter = (content) => {
...
function initLetterSurprise() {
  ...
  const spawnSurpriseHearts = (count) => {
...
function initMobilePolaroidSlider() {
  const polaroids = Array.from(document.querySelectorAll(".polaroid"));
...
function initRippleButtons() {
  const buttons = Array.from(document.querySelectorAll(".ripple-btn"));
...
function initParticleSystem() {
  ...
  const createParticle = (forcedHeart = false) => {
    const kind = forcedHeart ? "heart" : Math.random() > 0.55 ? "heart" : "spark";
    return {
